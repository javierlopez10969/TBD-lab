...
WHERE public.edificio_estacion.id_edificio = public.lugar.id_edificio AND NOT (public.edificio_estacion.id_edificio = public.contrato.id_edificio)
...
WHERE public.edificio_estacion.id_edificio = public.lugar.id_edificio AND NOT (public.edificio_estacion.id_edificio = public.contrato.id_edificio)
GROUP by public.edificio_estacion.nombre
HAVING count(public.lugar.id_lugar) IN (SELECT MAX(lugar_disponible.cantidad)
